...
class AdminTxtController extends AdminBaseController {
	...
	public function getIndex(){
		...
		$novel_query = DB::table('novels')
					...
					->leftJoin('genres', 'genres.id', '=', 'novels.genre_id')
					->orderBy('novels.created_at','DESC');
		try{
			$novel_data = $novel_query->get();
		}catch(Exception $e){
			$novel_data = '';
		}
		...
		$novel_list = '';
		if(!empty($novel_data)){
			foreach($novel_data as $novel_value){
				$novel_list[] = array(
					'id'			=> $novel_value->id,
					'title'			=> $novel_value->title,
					'created_at'	=> date('Y年m月d日',$novel_value->release_date),
					'genre'			=> $novel_value->name,
				);
			}
		}
		...
		return $this->layout->nest('content','admin.txt.index',$data);
